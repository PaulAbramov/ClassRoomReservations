namespace Reservations.Services
{
    ...
    public class ReservationsService : IReservationsService
    {
        ...
        public IEnumerable<ReservationItem> GetByDay(DateTime day, int hallNumber)
        {
            ...
            var hall = _queryAllLectureHalls.Execute().FirstOrDefault(x => x.Number == hallNumber);
            if (hall == null)
            {
                throw new ArgumentException("A hall with this number could not be found", "hallNumber");
            }

            reservations = _queryAll.Execute()
                                    .Where(reservation => reservation.From.Day == day.Day && reservation.To.Day == day.Day)
                                    .Where(reservation => reservation.Hall.Number == hall.Number)
                                    .OrderBy(reservation => reservation.From).ToList();
        ...
        public IEnumerable<HallFreeHoursStatisticsItem> GetHallsFreeHoursByDay(DateTime day)
        {
            ...
            var hallStatistics = new List<HallFreeHoursStatisticsItem>();
            var timeStart = new TimeSpan(8, 0, 0);
            var timeEnd = new TimeSpan(18, 0, 0);

            if (day.Day <= DateTime.Now.Day)
            {
                throw new ArgumentException("Date must be a day in the future", "day");
            }

            var reservationsForTheDay =_queryAll.Execute()
                                                                .Where(reservation => reservation.From.Day == day.Day && reservation.To.Day == day.Day);

            var allHalls = _queryAllLectureHalls.Execute();

            foreach (var hall in allHalls)
            {
                var reservations = reservationsForTheDay.Where(reservationToCheck => reservationToCheck.Hall.Number == hall.Number);

                if (reservations.Any())
                {
                    foreach (var reservation in reservations)
                    {
                        var startingTime = reservation.From.Hour - timeStart.Hours;
                        var endingTime = reservation.To.Hour - timeStart.Hours;
                        var timeOccupied = endingTime - startingTime;

                        hallStatistics.Add(new HallFreeHoursStatisticsItem
                        {
                            FreeHoursNumber = timeEnd.Hours - timeStart.Hours - timeOccupied,
                ...
                else
                {
                    hallStatistics.Add(new HallFreeHoursStatisticsItem
                    {
                        FreeHoursNumber = timeEnd.Hours - timeStart.Hours,
                        HallNumber = hall.Number
                    });
                }
            }
